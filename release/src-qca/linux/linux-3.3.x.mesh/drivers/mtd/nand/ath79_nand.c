...
#define CALDATA_REG_SIZE	0x100000
struct mtd_partition ath79_nand_caldata_partitions[] = {
        {
                name:           "caldata",
                size:           CALDATA_REG_SIZE,
                offset:         0,
        }
};

struct mtd_info *ath79_nand_mtd = NULL;
#else
#define CALDATA_REG_SIZE	0
#endif
			 ...
			 uint8_t * buf, int len, int page)
{
	...
	if (rw && ecc_enable) {
		...
		if (ecc_status & AR934X_NAND_ECC_ERROR) {
			...
			dev_info(ath79_priv->dev,
				"%s: Read [%d @ 0x%x] corrected. status = 0x%x\n",
				__func__, len, page, ecc_status);
			    ...
			    const uint8_t * buf, int page, int cached, int raw)
{
	...
	ath79_rw_page(mtd, chip, 1 /*read */ , ath79_priv->page_buf,
		      mtd->writesize, page);
	if ((mtd->ecc_stats.failed - stats.failed) > 0 || memcmp(buf, ath79_priv->page_buf, mtd->writesize))
		return -EIO;
#endif
	return 0;
}
...
#if defined(CONFIG_LSDK_NART_SUPPORT)
static void register_hook_for_caldata_region(struct mtd_info *mtd)
{
	if (!mtd)
		return;

	ath79_nand_caldata_partitions[0].offset = mtd->size - CALDATA_REG_SIZE;
	ath79_nand_caldata_partitions[0].size = CALDATA_REG_SIZE;
	ath79_nand_mtd = mtd;
}
#else
static inline void register_hook_for_caldata_region(struct mtd_info *mtd) { }
#endif

/***************************/
/****** ath79_nand_probe *****/
/***************************/
static int __devinit ath79_nand_probe(struct platform_device *pdev)
{
	struct resource *res;
...
#if defined(CONFIG_MTD_UBI)
	printk("MTD_BOOT_PART_SIZE %x MTD_CONFIG_PART_SIZE %x ath79_nand_partitions[0].size %lx mtd->erasesize %lx\n",
		MTD_BOOT_PART_SIZE, MTD_CONFIG_PART_SIZE, (unsigned long)ath79_nand_partitions[0].size, (unsigned long)mtd->erasesize);
	ath79_nand_partitions[1].offset = ALIGN(ath79_nand_partitions[0].size + MTD_CONFIG_PART_SIZE, mtd->erasesize);
	ath79_nand_partitions[1].size = mtd->size - ath79_nand_partitions[1].offset - CALDATA_REG_SIZE;
#else	/* !CONFIG_MTD_UBI */
	offs = MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE;
	ramtd_nand_read(mtd, offs, sizeof(hdr), (size_t *)&alloc_size, (u_char *)(&hdr));
	...
	if (rfs_offset != 0) {
		ath79_nand_partitions[4].offset = MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE + rfs_offset;
		ath79_nand_partitions[4].mask_flags |= MTD_WRITEABLE;
		if (mtd->size > 0x800000) {
			ath79_nand_partitions[3].size = mtd->size - (MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE) - MTD_JFFS2_PART_SIZE;
			ath79_nand_partitions[4].size = mtd->size - ath79_nand_partitions[4].offset - MTD_JFFS2_PART_SIZE;
			ath79_nand_partitions[5].offset = mtd->size - MTD_JFFS2_PART_SIZE;
		} else {
			ath79_nand_partitions[3].size = mtd->size - (MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE);
			ath79_nand_partitions[4].size = mtd->size - ath79_nand_partitions[4].offset;
			ath79_nand_partitions[5].name = ath79_nand_partitions[6].name;
			ath79_nand_partitions[5].size = ath79_nand_partitions[6].size;
			ath79_nand_partitions[5].offset = ath79_nand_partitions[6].offset;
		}
		printk(KERN_NOTICE "partion 3: %x %x\n", (unsigned int)ath79_nand_partitions[3].offset, (unsigned int)ath79_nand_partitions[3].size);
		printk(KERN_NOTICE "partion 4: %x %x\n", (unsigned int)ath79_nand_partitions[4].offset, (unsigned int)ath79_nand_partitions[4].size);
	}
#endif

	/* Register the partitions */
	add_mtd_partitions(mtd, ath79_nand_partitions, ARRAY_SIZE(ath79_nand_partitions));

	register_hook_for_caldata_region(mtd);
