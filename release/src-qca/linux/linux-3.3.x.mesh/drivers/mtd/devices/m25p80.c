...
static struct mtd_partition ath79_spi_partitions[] = {
        {
                name:           "Bootloader",
                ...
                offset:         MTDPART_OFS_APPEND,
#ifdef CONFIG_MTD_PLC_PARTS
        }, {
                name:           "plc",
                size:           MTD_PLC_PART_SIZE,
                offset:         MTDPART_OFS_APPEND,
#endif
#ifdef CONFIG_MTD_JFFS2_PARTS
        }, {
                name:           "jffs2",
...
static int __devinit m25p_probe(struct spi_device *spi)
{
	...
	struct mtd_partition *parts = ath79_spi_partitions;
	struct flash_platform_data fake_data = {
		.name = "ath79-nor0",
		.parts = ath79_spi_partitions,
		.nr_parts = ARRAY_SIZE(ath79_spi_partitions),
	...
	if (!data) {
		...
		parts[3].size = flash->mtd.size - (MTD_BOOT_PART_SIZE 
...
#ifdef CONFIG_MTD_PLC_PARTS
		parts[5].offset = parts[3].offset + parts[3].size;
#ifdef CONFIG_MTD_JFFS2_PARTS
		if (jffs2_size > 0) {
			parts[6].offset = flash->mtd.size - jffs2_size;
		} else {
			parts[6].name = parts[7].name;
			parts[6].size = parts[7].size;
			parts[6].offset = parts[7].offset;
		}
#endif
#else
#ifdef CONFIG_MTD_JFFS2_PARTS
		...
		add_mtd_partitions(&flash->mtd, parts, data->nr_parts);
 ...
 */
static inline int reset_4byte(struct m25p *flash)
{
	struct spi_transfer t[2];
	struct spi_message m;
	char command[2];

	if (flash->addr_width != 4 || ex4b == 1)
		return 1;
...
int __reset_4byte(int wait)
{
	int ret = -1, i = 0;

	if (pflash)
		ret = reset_4byte(pflash);

	if (ret == -1)
		printk(KERN_EMERG "__reset_4byte FAIL! System will not automatically reboot...\n");
	else if (ret == 0) {
		printk(KERN_EMERG "System will rebooting in %d secs for __reset_4byte...\n", wait);
		for (i = 0; i < (wait * 10); i++)
			mdelay(100);
	}
