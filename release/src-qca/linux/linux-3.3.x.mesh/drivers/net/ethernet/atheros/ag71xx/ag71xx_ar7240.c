...
#if 1 // ASUS PL-N12 switch
static int
ar7240_set_port_power(struct switch_dev *dev, const struct switch_attr *attr,
		   struct switch_val *val)
{
	struct ar7240sw *as = sw_to_ar7240(dev);
	struct mii_bus *mii = as->mii_bus;
	...
	t = ar7240sw_phy_read(mii, port-1, MII_BMCR);
	if (val->value.i) // power on
		t &= ~(1<<11);
	else
		t |= (1<<11);
	ar7240sw_phy_write(mii, port-1, MII_BMCR, t);
	return 0;
}

static int
ar7240_get_port_power(struct switch_dev *dev, const struct switch_attr *attr,
		   struct switch_val *val)
{
	struct ar7240sw *as = sw_to_ar7240(dev);
	struct mii_bus *mii = as->mii_bus;
...
static struct switch_attr ar7240_port[] = {
#if 1 // ASUS PL-N12 switch
	{
		...
		.set = ar7240_set_port_power,
		.get = ar7240_get_port_power,
	},
#endif
};
