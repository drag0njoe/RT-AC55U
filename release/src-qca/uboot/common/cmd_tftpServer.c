...
unsigned int
__checksum(unsigned int *data, unsigned int size, int byteswap)
{
	unsigned checksum = 0;
	int i;
 ...
 * 	> 0:	correct boot code
 */
int check_uboot(ulong buf, ulong total_len)
{
#if defined(CONFIG_ATH_NAND_BR)
	const unsigned int addr_tbl[2] = { 0xbd004000, 0xa0100000 };
	uint32_t addr, csum;
	unsigned int nr_bcode = 0;
	unsigned int len = total_len;
	struct qca_bcode_hdr_s {
		uint32_t entry_addr;
		uint32_t load_addr;
		uint32_t length;	/* include header itself */
		uint32_t checksum;
	} *hdr;
	const unsigned int hsize = sizeof(*hdr);

	while (len > 0 && nr_bcode < 2) {
		hdr = (struct qca_bcode_hdr_s*) buf;

		debug("check stage%d ...\n", nr_bcode);
		addr = addr_tbl[nr_bcode];
		if (ntohl(hdr->entry_addr) != addr || ntohl(hdr->load_addr) != addr) {
			printf("Invalid entry/load address %08x/%08x! (expected %08x/%08x)\n",
				ntohl(hdr->entry_addr), ntohl(hdr->load_addr), addr, addr);
			return -1;
		}

		csum = __checksum((unsigned int*) (buf + hsize), ntohl(hdr->length) - hsize, 0);
		if (csum != ntohl(hdr->checksum)) {
			printf("Checksum mismatch %08x -> %08x\n", ntohl(hdr->checksum), csum);
			return -2;
		}

		buf += ntohl(hdr->length);
		len -= ntohl(hdr->length);
		nr_bcode++;
	}

	printf("check boot code: OK\n");
	return 1;
#else	/* !CONFIG_ATH_NAND_BR */
	/* FIXME: We can't verify u-boot.bin due to it lacks checksum information.  */
	printf("skip boot cood verification!!!\n");
	return 1;
#endif	/* CONFIG_ATH_NAND_BR */
}
...
int program_bootloader(ulong addr, ulong len)
{
	char *argv[2];
	char buf[16];
	unsigned int base = CFG_FLASH_BASE;
	...
	if (!memcmp((void*)addr, trx_magic, 4)) {
		sprintf(buf, "0x%08x", addr);
		argv[0] = NULL;
		argv[1] = buf;
		if ((rc = check_trx(2, argv)) <= 0) {
			printf("# trx checksum error (%d) #\n", rc);
			return -1;
		}
		...
		printf("strip trx header\n");
	}

#if defined(CONFIG_ATH_NAND_BR)
	base -= CFG_FLASH_BASE;
	if ((rc = check_uboot(addr, len)) <= 0) {
		printf("## stage1/2 checksum error (%d) ##\n", rc);
		return -2;
	}
#endif

	printf("program: from(%08x), to(%08x), size(%x)\n", addr, base, len);
#if defined(UBOOT_STAGE1)
	...
	if (len <= CFG_BOOTSTAGE1_SIZE) {
		...
		ranand_write_stage2(CFG_LOAD_ADDR, len);
	}
#else
#if defined(CONFIG_ATH_NAND_BR)
	ranand_write_bootloader(addr, len);
#else
...
#if defined(DUAL_TRX)
static int load_asus_firmware(cmd_tbl_t *cmdtp, int argc, char *argv[])
{
	int r;
...
static int load_lsdk_nart_firmware(cmd_tbl_t *cmdtp, int argc, char *argv[])
{
	if (check_trx(argc, argv) <= 0) {
		printf(" \nInvalid LSDK NART firmware!!\n\n");
			return 1;
	}
...
int do_tftpd(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
...
#endif /* ! UBOOT_STAGE1 */
	{
		...
		switch (addr) {
#if defined(LSDK_NART_SUPPORT)
		case CFG_LSDKNART_ADDR:
			leds_on();
			ret = load_lsdk_nart_firmware(cmdtp, argc, argv);
			puts("\nFallback to ASUS firmware!\n");
			/* fall through, If LSDK NART firmware doesn't work, load ASUS firmware instead. */
#endif
		default:
			leds_off();
			power_led_on();
...
static void TftpHandler(uchar * pkt, unsigned dest, unsigned src, unsigned len)
{
	...
	case TFTP_RRQ:
		...
		if (!memcmp(pkt, asuslink, 13) || !memcmp(pkt, maclink, 13)) {
			...
			PrintIPAddr("Firmware Restoration assigns IP address: ", NetOurIP);
		}
...
static void RAMtoFlash(void)
{
	...
	else
	{
		...
		printf("parseflag %d\n", parseflag);
		if (parseflag == 1) {
			if ((rc = program_bootloader((ulong)ptr, copysize)))
				parseflag = -1;
		}
