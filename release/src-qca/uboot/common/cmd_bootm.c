...
int verify_kernel_image (int argc, char *argv[], ulong *pAddr, ulong *pData, ulong *pLen)
{
...
#else
		unsigned char raw[] = {/*0*/ 0xFF, 0x00, 0x00, 0x10, /*4*/ 0x00, 0x00, 0x00, 0x00, /*8*/ 0xFD, 0x00, 0x00, 0x10, /*C*/ 0x00, 0x00, 0x00, 0x00};
		if (memcmp((void*)hdr, raw, sizeof(raw)) == 0) {
			printf ("## Try with fake header ##\n");
			memset(hdr, 0, sizeof(*hdr));
			hdr->ih_magic = htonl(IH_MAGIC);
			hdr->ih_size  = htonl(CFG_BOOTLOADER_SIZE);
			hdr->ih_load  = htonl(addr);
			hdr->ih_ep    = htonl(addr);
			hdr->ih_os    = IH_OS_LINUX;
			hdr->ih_arch  = IH_CPU_MIPS;
			hdr->ih_comp  = IH_COMP_NONE;
			hdr->ih_type  = IH_TYPE_STANDALONE;
			strcpy((char*) hdr->u.ih_name, "Fake Header");
			lastAddr = addr;
			lastData = addr;
			lastLen  = CFG_BOOTLOADER_SIZE;
			goto skip;
		} else
#endif	/* __I386__ */
	    {
	...
	if (addr >= CFG_FLASH_BASE) {
		...
		ra_flash_read((uchar*) load_addr, data, len);
		data = load_addr;
	}
#endif
	if (verify) {
...
extern void mips_icache_flush_ix(void);
	/*
	...
	case IH_COMP_NONE:
		if(ntohl(hdr->ih_load) == addr) {
			...
			printf ("   Loading %s from %p to %p ... ", name, (uchar *)data, (void *) ntohl(hdr->ih_load));
			memmove ((void *) ntohl(hdr->ih_load), (uchar *)data, len);
#endif	/* CONFIG_HW_WATCHDOG || CONFIG_WATCHDOG */
		}
		break;
#if !defined(COMPRESSED_UBOOT) && !defined(SHRINK_UBOOT)
	case IH_COMP_GZIP:
	...
	case IH_COMP_BZIP2:
		...
		break;
#endif /* CONFIG_BZIP2 */
#endif /* #if !COMPRESSED_UBOOT && !SHRINK_UBOOT */
#ifdef CONFIG_LZMA
	case IH_COMP_LZMA:
		printf ("   Uncompressing %s ... ", name);
