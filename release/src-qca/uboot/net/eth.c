...
#    endif
	...
	if (!eth_devices) {
		...
		const unsigned char empty_mac[6] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
#endif
#endif
...
#if defined(ASUS_PRODUCT)
#if defined(UBOOT_STAGE1)
			memcpy(mac, default_ethaddr, 6);
#else
			//get Ethernet mac address from flash
			memset(mac, 0, sizeof(mac));
			r = ra_factory_read(mac, RAMAC0_OFFSET, 6);

			//if mac address is invalid, flash is empty, or mac address is zero, use default mac address
			if (r || (mac[0] & 1) || !memcmp(mac, empty_mac, 6) || !memcmp(mac, "\0\0\0\0\0\0", 6)) {
...
int eth_init(bd_t *bis)
{
	...
	char *preferred_nic;
	preferred_nic=getenv("preferred_nic");
