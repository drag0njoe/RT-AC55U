...
//#if !defined(CONFIG_ATH_EMULATION)
	...
	// set UART0_SOUT
	val = ath_reg_rd(GPIO_OE_ADDRESS) & (~(1 << 10));	//clear OE bit for setting OUTPUT
	ath_reg_wr(GPIO_OE_ADDRESS, val);
	ath_reg_rmw_set(GPIO_OUT_FUNCTION2_ADDRESS,
			GPIO_OUT_FUNCTION2_ENABLE_GPIO_10_SET(0x16));
	// set UART_SIN
	val = ath_reg_rd(GPIO_OE_ADDRESS) | (1 << 9);	//set OE bit for setting INPUT
	ath_reg_wr(GPIO_OE_ADDRESS, val);
	...
	/* Make sure GPIO##18,#20 is not turn on here. */
	ath_reg_wr(GPIO_OUT_ADDRESS, 0x2f | ((1U << 20) | (1U << 18)));
#endif
#endif	/* ! ASUS_PRODUCT */
