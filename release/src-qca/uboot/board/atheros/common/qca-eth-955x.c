...
ath_gmac_send(struct eth_device *dev, volatile void *packet, int length)
{
	...
	if (mac->next_tx > (NO_OF_TX_FIFOS + 1)) {
		printf("%s: invalid tx[%d]", mac->next_tx);
		return -1;
	}

	f = mac->fifo_tx[mac->next_tx];
	f->pkt_size = length;
	f->res1 = 0;
...
#if defined(ASUS_PRODUCT)
	ath_gmac_reg_wr(mac, ATH_MAC_FIFO_CFG_5, 0x7efcf); // also enable broadcast/multicast
#else
...
static void ath_gmac_get_ethaddr(struct eth_device *dev)
{
	...
	if ((r = ra_factory_read(mac, RAMAC0_OFFSET, 6))) {
		printf("%s: read MAC address from offset %x for dev %s fail! (r = %d)\n",
			__func__, RAMAC0_OFFSET, dev->name, r);
	}

	/* Use fixed address if the above address is invalid */
	if (r || (mac[0] & 1) || (mac[0] == 0xff && mac[5] == 0xff))
#else /* !ASUS_PRODUCT */
...
#endif /* !CONFIG_ATH_EMULATION */
	{
		...
		printf("Fetching MAC Address from factory\n", __func__);
	}
}
