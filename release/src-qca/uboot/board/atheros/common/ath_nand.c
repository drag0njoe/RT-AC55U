...
#define DEBUG_CORRECTABLE_ERROR_ON_WRITE

#define ENOMEM		12
#define EINVAL		22

#define DRV_NAME	"nand0"
...
ath_nand_vend_data_t ath_nand_arr[] = {
	...
	{ 0x92, 0xf1, 0x80, 4, },	// Zentel A5U1GA31ATS-BC, 8-bit [128MB]
...
#if defined(DEBUG_CORRECTABLE_ERROR_ON_WRITE)
uint8_t	raw_buf[24 << 10] __attribute__((aligned(4096)));
#endif
...
int ranand_set_sbb_max_addr(loff_t addr)
{
...
void ath_nand_set_blk_state_wrap(struct mtd_info *mtd, loff_t b, unsigned state)
{
	ath_nand_set_blk_state(mtd, b, state);
}
...
#define DDR_WB_FLUSH_USB_ADDRESS		0x180000a4
		...
		if (ecc_needed && (ecc & ATH_NF_ECC_ERROR)) {
			...
			if (mlc_retry < ATH_MLC_RETRY) {
				...
				printk("%s: %s uncorrectable errors @ page %x. ecc = 0x%x\n",
					__func__, err[rd], addr0 >> 16, ecc);
				mtd->ecc_stats.failed++;
				return ATH_NF_STATUS_OK;
			}
		}
		else if (ecc_needed && (ecc & ATH_NF_ECC_CTRL_ERR_CORR)) {
			printk("%s: %s corrected @ page %x, ecc 0x%x\n",
				__func__, err[rd], addr0 >> 16, ecc);
...
{
#if defined(DEBUG_UNCORRECTABLE_BLOCK)
	unsigned	*buf = v, *end;

	iodbg("____ Dumping %d bytes at 0x%p 0x%lx_____\n", count, buf, (ulong)addr);

	count = (count >= 0x50)? 0x50:count;	/* 0x50 = sizeof(EC/VID header) + 16 */
	end = buf + (count / sizeof(*buf));

	for (; buf && buf < end; buf += 10, addr += 40) {
		printk("%08lx: %08x %08x %08x %08x %08x %08x %08x %08x %08x %08x\n",
			(unsigned)addr, buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7], buf[8], buf[9]);
	}
...
#if defined(DEBUG_CORRECTABLE_ERROR_ON_WRITE)
	struct mtd_ecc_stats s;
#endif

	stats = mtd->ecc_stats;
	*iodone = 0;

	while (len && addr < mtd->size) {
		uint32_t c, ba0, ba1;
		...
		if (ath_nand_block_isbad(mtd, addr)) {
			...
			printf("%s: %lx exceed max_addr %lx\n", __func__, (ulong) addr, (ulong) max_addr);
			ret = -ENOSPC;
			break;
		}
		...
		if (rd) {
			...
			memcpy(ath_nand_io_buf + c, buf, iolen);
			ecc_needed = 1;
		}
	...
	/* verify write result. */
	for (len = *iodone, addr = vaddr, buf = vbuf; !rd && verify_ok && len > 0; ) {
		uint32_t c, ba0, ba1;
...
#endif
		pa = (void *)virt_to_phys(ath_nand_io_buf);
		ret = ath_nand_rw_page(sc, 1, ba0, ba1, mtd->writesize, pa, ecc_needed);
		flush_cache((unsigned)ath_nand_io_buf, mtd->writesize);
		if (ret != ATH_NF_STATUS_OK || memcmp(buf, ath_nand_io_buf + c, iolen))
			verify_ok = 0;
#if defined(DEBUG_CORRECTABLE_ERROR_ON_WRITE)
		if (!(mtd->ecc_stats.failed - s.failed) && (mtd->ecc_stats.corrected - s.corrected)) {
			/* If correctable error found, read same address/length w/o ECC, dump bit-fliped bytes. */
			pa = (void *)virt_to_phys(raw_buf);
			ret = ath_nand_rw_page(sc, 1, ba0, ba1, mtd->writesize, pa, 0);
			flush_cache((unsigned)raw_buf, mtd->writesize);
			if (ret != ATH_NF_STATUS_OK) {
				printf("%s: read raw data fail. (ret %d)\n", __func__, ret);
			} else {
				int i, j;
				uint32_t *p32, *q32;
				uint8_t *p, *q;

				p32 = (uint32_t*)ath_nand_io_buf;
				q32 = (uint32_t*)raw_buf;
				for (i = 0; i < mtd->writesize; i += 16, p32 += 4, q32 += 4) {
					if (!(p32[0] ^ q32[0]) && !(p32[1] ^ q32[1]) &&
					    !(p32[2] ^ q32[2]) && !(p32[3] ^ q32[3]))
						continue;

					p = (uint8_t*)ath_nand_io_buf + i;
					q = (uint8_t*)raw_buf + i;
					for (j = 0; j < 16; ++j, ++p, ++q) {
						if (!j)
							printf("%08X:  ", i);
	...
	if (!verify_ok || mtd->ecc_stats.failed - stats.failed)
		return -EBADMSG;
...
ath_nand_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	for (j = 0, i = s_first; i < mtd->size && j < n; i += mtd->erasesize) {
		uint32_t ba0, ba1;
...
ath_nand_write_oob(struct mtd_info *mtd, loff_t to, struct mtd_oob_ops *ops)
{
	...
	if (ops->mode == MTD_OOB_AUTO) {
		/* read existing oob */
		if (ath_nand_rw_oob(mtd, 1 /* read */, to, &rops) || rops.oobretlen != rops.ooblen) {
	...
	ath_nand_set_blk_state(mtd, to, ATH_NAND_BLK_DONT_KNOW);
...
ath_nand_block_markbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	ofs &= ~mtd->erasesize_mask;	/* aligned to block address */
	printf("offset %lx (block %lx)\n", (ulong) ofs, (ulong) ofs >> mtd->erasesize_shift);
	if ((ret = nand_read_raw(mtd, buf, ofs, 0, sizeof(buf))) != 0) {
		printf("read oob from %lx fail. (ret %d)\n", ofs, ret);
		return 0;
	}
	if (buf[0] != 0xFF && buf[0] != SW_BAD_BLOCK_INDICATION) {
		printf("incorrect block indication byte %2x, skip\n", buf[0]);
		return 0;
	}
	if ((ret = nand_erase(mtd, ofs, mtd->erasesize)) != 0) {
		printf("erase offset %lx fail. (ret %d)\n", ofs, ret);
		return 0;
	}
	memset(buf, 0xff, sizeof(buf));
	buf[0] = SW_BAD_BLOCK_INDICATION;
	...
	ops.ooblen = 1;
	ops.mode =  MTD_OOB_RAW;
	if ((ret = mtd->write_oob(mtd, ofs, &ops)) != 0)
		printf("write page %lx fail. (ret %d)\n", ofs, ret);
	ofs += mtd->writesize;
	if ((ret = mtd->write_oob(mtd, ofs, &ops)) != 0)
		printf("write page %lx fail. (ret %d)\n", ofs, ret);
...
static ulong ath_nand_probe(void)
{
	...
	for (i = 0; i < 4; ++i)
		id[i] = sc->__id.byte_id[3-i];
	if (id[0] == 0x98 && id[1] == 0xF1 && id[2] == 0x80 && id[3] == 0x15) {
		mtd->oobsize = 128;
		mtd->oobavail = mtd->oobsize;
	}
	...
	printf(	"====== NAND Parameters ======\n"
		...
		sc->nf_ctrl, mtd->writesize, mtd->erasesize, mtd->oobsize, (ulong)(mtd->size >> 20));

	if ((r = add_mtd_device(mtd)) != 0)
		printf("%s: register mtd device fail. (code %d)\n", __func__, r);
