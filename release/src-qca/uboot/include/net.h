...
typedef enum { BOOTP, RARP, ARP, TFTP, DHCP, PING, DNS, NFS, CDP, NETCONS, SNTP, ATHRHDR, TFTPD } proto_t;
#else
typedef enum { BOOTP, RARP, ARP, TFTP, DHCP, PING, DNS, NFS, CDP, NETCONS, SNTP, TFTPD } proto_t;
#endif
...
/* Print IP information with/without message.
 * @IP:	32-bit IP address in network endian
 */
static inline void PrintIPAddr(char *msg, ulong IP)
{
	if (msg && *msg != '\0')
		puts (msg);
	print_IPaddr(IP);
